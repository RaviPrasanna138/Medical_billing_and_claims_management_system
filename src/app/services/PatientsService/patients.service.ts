...
export class PatientsService {
  ...
  patientURL: string = 'http://localhost:8085/api/patients';
  ...
  registerPatients(patients: Patients): Observable<Patients> {
    return this.http.post<Patients>(this.patientURL + "/addpatients", patients);
  }
  ...
  updatePatient(patient: Patients): Observable<Patients> {
    ...
    if (token) {
      ...
      console.log(headers);
      return this.http.put<Patients>(this.patientURL + '/updatepatient/' + patient.patientId, patient, { headers });
    }
    else {
      return new Observable<Patients>;
    }

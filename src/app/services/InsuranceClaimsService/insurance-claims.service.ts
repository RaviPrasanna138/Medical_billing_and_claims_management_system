...
export class InsuranceClaimsService {
  ...
  adminURL: string = "http://localhost:8085/api/admin";
  claimURL: string = "http://localhost:8085/api/insuranceclaims";
  companyURL: string = 'http://localhost:8085/api/insurancecompany';
  ...
  getAllInsuranceClaims(): Observable<InsuranceClaims[]> {
    ...
    if (token) {
      ...
      console.log(headers);
      return this.http.get<InsuranceClaims[]>(this.adminURL + '/getallclaims', { headers });
    }
    else {
  ...
  getClaimForPatient(patientName: string): Observable<InsuranceClaims[]> {
    ...
    if (token) {
      ...
      console.log(headers);
      return this.http.get<InsuranceClaims[]>(this.claimURL + `/getclaims/${patientName}`, { headers })
        .pipe(
          catchError((error: any) => {
            console.error('Error adding claim:', error);
            return throwError(error); // Re-throw the error to be handled by the caller

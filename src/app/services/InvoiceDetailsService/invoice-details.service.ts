...
export class InvoiceDetailsService {
  ...
  invoiceURL: string = "http://localhost:8085/api/invoicedetails"
  ...
  createNewInvoice(invoice: InvoiceDetails, patientId: number): Observable<InvoiceDetails> {
    ...
    if (token) {
      ...
      return this.http.post<InvoiceDetails>(this.invoiceURL + `/add/${patientId}`, invoice, { headers })
        .pipe(
          catchError((error: any) => {
  ...
  getInvoicesByPatientName(patientName: string): Observable<InvoiceDetails[]> {
    ...
    if (token) {
      ...
      return this.http.get<InvoiceDetails[]>(this.invoiceURL + `/getallbypatientname/${patientName}`,{headers})
        .pipe(
          catchError((error: any) => {
            console.error('Error searching patient:', error);
            return throwError(error); // Re-throw the error to be handled by the caller
